...
def isPassive(sentence):
    ...
    if tags.count("VBN") == 0:  # no PP, no passive voice.
        ...
        tags.count("VBN") == 1 and "been" in words
        ...
        for end in pos:
            ...
            if verbspos != []:  # if there are no verbs in between, it's not passive
                for i in verbspos:
                    if (
                        sentchunk[i].lower() not in beforms
...
def search_params():
    SEARCHLIST = [
        ...
        "mustang unicorn",
        "scrook rgerkin neuron",
        "prancercise philosophy",
        "play dough delicious deserts",
    ]
    _, ses = engine_dict_list()
    WEB = len(
        ses
    )  # how many search engines to include (many possible- google google scholar bing yahoo)
...
def search_known_corpus():
    ...
    LINKSTOGET.append(
        str("https://academic.oup.com/beheco/article-abstract/29/1/264/4677340")
    )
    ...
    LINKSTOGET.append(
        str(
            "https://scholar.google.com/scholar?hl=en&as_sdt=0%2C3&q=Patrick+mcgurrin+ASU&btnG="
        )
    )
    LINKSTOGET.append(
        str("https://scholar.google.com/citations?user=GzG5kRAAAAAJ&hl=en&oi=sra")
    )
    LINKSTOGET.append(
        str(
    ...
    LINKSTOGET.append(
        str("https://scholar.google.com/citations?user=2agHNksAAAAJ&hl=en&oi=sra")
    )
    # _, ses = engine_dict_list()
    WEB = 1
