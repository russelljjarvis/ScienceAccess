...
def author_to_urls(NAME):
	...
	#: dissem.in
	#['http://doi.org/10.2147/tcrm.s121804'] 1
	#['http://dx.doi.org/10.3389/fninf.2016.00008'] 1
	#['http://dx.doi.org/10.1186/1471-2202-16-s1-p178'] 1
	#2021-02-14 21:16:44.508 Starting new HTTPS connection (1): api.u
...
def take_url_from_gui(NAME,tns):
	...
	for doi_ in tqdm(dois,title='Scrape in Progress. Please Wait.'):
		r = str('https://api.unpaywall.org/v2/')+str(doi_)+str('?email=rjjarvis@asu.edu')
