...
lods = []
for i, j, k in zip(
    bio_chem, [str("Comparison Data") for i in range(0, len(bio_chem))], biochem_labels
):
...
if author_name:
    ...
    fig0.update_layout(
        title_text="Scholar scraped {0} Versus Art Corpus".format(author_name),
        width=900,
        height=900,
    )  # , hovermode='x')
    ...
    lods = []
    for i, j, k in zip(
        standard_sci, [str("S Phatak") for i in range(0, len(ar))], scraped_labels
    ):
        lods.append({"Reading_Level": i, "Origin": j, "Web_Link": k})
    df1 = pd.DataFrame(lods)
    df = pd.concat([df1, df0])
    ...
    fig0.update_layout(
        title_text="Scholar S Phatak Versus Art Corpus", width=900, height=600
    )  # , hovermode='x')
...
fig = ff.create_distplot(
    [x1, x2, x3], group_labels, bin_size=1, colors=colors, rug_text=rt
)
