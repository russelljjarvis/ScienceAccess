...
def process_batch(batch: dict):
    ...
    with gzip.open(batch["input_metadata_path"], "rb") as gz, open(
        ...
        for line in tqdm(f.readlines()):
            ...
            if (
                mag_field_of_study and "Medicine" in mag_field_of_study
            ):  # TODO: <<< change this to your filter
                paper_ids_to_keep.add(paper_id)
                f_out.write(line)
            if (
                mag_field_of_study and "Neuroscience" in mag_field_of_study
            ):  # TODO: <<< change this to your filter
                paper_ids_to_keep.add(paper_id)
                f_out.write(line)
            if (
                mag_field_of_study and "Engineering" in mag_field_of_study
            ):  # TODO: <<< change this to your filter
                paper_ids_to_keep.add(paper_id)
                f_out.write(line)
...
if __name__ == "__main__":
    ...
    # PDF_PARSES_OUTPUT_DIR = 'pdf_parses/engineering/'
